...
const israelRuCopy = {
  ...copy.ru,
  metaTitle: 'AdPilot для русскоязычного бизнеса в Израиле',
  metaDescription: 'Бесплатный стратегический разбор бизнеса в Telegram и ежедневный контроль Meta Ads Алексом. Без карты и подключения рекламного кабинета до оплаты.',
  nav: ['Как работает', 'Для бизнеса', 'Контроль', 'Цена', 'Вопросы'],
  navTry: 'Получить разбор',
  heroBadge: 'AdPilot для русскоязычного бизнеса в Израиле',
  heroTitle: 'Кто сегодня проверял вашу рекламу?',
  heroLead: 'Алекс анализирует показатели Meta Ads, объясняет, почему меняются обращения и расходы, и предлагает конкретные корректировки. Бюджет, пауза и другие критические изменения — только после вашего подтверждения.',
  primary: 'Получить бесплатный стратегический разбор бизнеса',
  secondary: 'Посмотреть, как работает Алекс',
  offerNote: 'Без карты и подключения Meta · 10–15 минут в Telegram · полный доступ — €150/месяц',
  heroFacts: ['На русском языке', 'Для услуг, товаров и локальных заведений', 'Рекламный бюджет отдельно'],
  chat: {
    online: 'онлайн',
    user: 'Обращений в WhatsApp стало меньше, хотя бюджет не меняли.',
    answer: '<strong>Что изменилось:</strong> за последние три дня стоимость обращения выросла с ₪42 до ₪67.',
    settings: '<strong>Возможная причина</strong><br>Частота показов выросла, а текущий креатив работает уже 18 дней. Сначала стоит проверить его усталость и конверсию после клика.',
    report: '<strong>Предлагаю</strong><br>Оставить текущий бюджет и подготовить новый вариант объявления.',
    approve: 'Подтвердить · Не менять · Задать вопрос'
  },
  trust: [
    ['Бесплатный разбор', 'Без карты и рекламного кабинета'],
    ['Официальный OAuth', 'Пароль Meta не передаётся'],
    ['Ваше подтверждение', 'Перед критическими изменениями'],
    ['€150 / месяц', 'Stripe до отмены или крипто на 30 дней']
  ],
  howKicker: 'Бесплатный вход',
  howTitle: 'Сначала получите пользу. Потом решайте об оплате',
  howLead: 'Бесплатный стратегический разбор бизнеса не требует карты, доступа к Meta или рекламного бюджета.',
  steps: [
    ['Расскажите о бизнесе', 'Ответьте в Telegram на вопросы о продукте, клиентах, географии и способе получения обращений.'],
    ['Получите персональный разбор', 'Алекс покажет первый сегмент, рекламное обещание, узкое место воронки и следующий стратегический шаг.'],
    ['Подключите полную систему', 'Если разбор полезен, оплата открывает подготовку кампании, креативы, запуск и дальнейший контроль показателей.']
  ],
  demoKicker: 'Что вы получите бесплатно',
  demoTitle: 'Выводы по вашему бизнесу, а не универсальный шаблон',
  demoLead: 'Разбор учитывает продукт, географию, способ получения обращений и рекламную цель. Он показывает направление, но не открывает платные настройки до оплаты.',
  featuresKicker: 'Знакомые ситуации',
  featuresTitle: 'Один Алекс — разные виды бизнеса',
  featuresLead: 'Пример помогает узнать свою ситуацию, но не ограничивает продукт одной нишей.',
  features: [
    ['01 / Услуги и ремонт', 'Когда обращения в WhatsApp становятся дороже', 'Алекс сопоставляет расход, клики, частоту показов и путь до обращения, чтобы показать, что проверить первым.'],
    ['02 / Beauty', 'Когда в расписании появились свободные окна', 'Алекс помогает понять, требует ли внимания оффер, креатив, аудитория или переход от объявления к записи.'],
    ['03 / Товарный бизнес', 'Когда клики есть, а продажи непонятны', 'Алекс разделяет рекламные сигналы и предлагает следующий тест без выдуманных обещаний окупаемости.'],
    ['04 / Кафе и доставка', 'Когда старое объявление продолжает тратить бюджет', 'Алекс замечает изменения показателей и предлагает обновить акцию, сообщение или креатив после вашего подтверждения.']
  ],
  controlKicker: 'Ежедневная работа',
  controlTitle: 'Алекс анализирует. Вы принимаете решение',
  controlLead: 'Бесплатный разбор показывает направление. После оплаты AdPilot помогает подготовить кампанию и продолжает работать с её показателями.',
  controlChecks: ['Проверяет рекламные показатели каждые 6 часов', 'Объясняет важные изменения понятным языком', 'Присылает отчёты и важные уведомления в Telegram', 'Предлагает корректировки с объяснением причины', 'Применяет критические изменения после подтверждения'],
  compareHead: ['Алекс делает', 'Вы решаете'],
  compare: [['Анализирует анкету и метрики', 'Подключать ли рекламный кабинет'], ['Готовит кампанию и объявления', 'Принимать ли настройки'], ['Проверяет показатели', 'Менять ли бюджет или ставить на паузу'], ['Предупреждает о проблемах', 'Подтверждать ли предложенное действие']],
  priceKicker: 'Один тариф',
  priceTitle: 'Не разовый AI-текст, а продолжающаяся работа с рекламой',
  priceLead: 'Если бесплатный стратегический разбор бизнеса полезен, полный доступ превращает направление в настройки, объявления, запуск и дальнейший контроль.',
  previewBadge: 'Бесплатный стратегический разбор бизнеса до оплаты',
  priceItems: ['Точные настройки аудитории, географии, бюджета и CTA', 'Создание кампаний в Meta и TikTok после проверки', 'Варианты объявлений и инструменты для визуалов', '100 AI-кредитов на 30 дней', 'Проверка показателей каждые 6 часов', 'Отчёты и важные уведомления в Telegram', 'Критические изменения только после подтверждения', 'Поддержка внутри бота'],
  exclusions: 'Stripe: автоматическое списание €150 каждый месяц до отмены. Криптовалюта: разовая оплата €150 за 30 дней без автопродления. Рекламный бюджет оплачивается отдельно.',
  priceButton: 'Сначала получить бесплатный разбор',
  faqKicker: 'Вопросы',
  faqTitle: 'Коротко о работе в Израиле',
  faqs: [
    ['Подходит ли AdPilot бизнесу в Израиле?', 'Да. В бесплатном стратегическом разборе бизнеса учитываются география, способ получения обращений и особенности вашего предложения. Интерфейс и общение с Алексом доступны на русском языке.'],
    ['Работает ли AdPilot с обращениями в WhatsApp?', 'WhatsApp можно выбрать как рекламное направление для получения обращений, если рекламные ресурсы и формат кампании Meta это поддерживают. AdPilot не читает переписку и не отвечает клиентам в WhatsApp.'],
    ['Работает ли система с рекламным аккаунтом в шекелях?', 'AdPilot использует валюту подключённого рекламного аккаунта и показывает рекламные показатели в ней. Подписка AdPilot оплачивается отдельно в EUR.'],
    ['Подходит ли система только для услуг?', 'Нет. AdPilot подходит для услуг, товарных проектов, локальных заведений и онлайн-бизнеса. Конкретная стратегия строится по вашим ответам и данным.'],
    ['Алекс самостоятельно изменит бюджет?', 'Нет. Алекс анализирует данные и предлагает корректировки. Бюджет, пауза и другие критические изменения выполняются только после вашего подтверждения.'],
    ['Гарантирует ли AdPilot заявки или продажи?', 'Нет. Результат зависит от предложения, рынка, бюджета и рекламных данных. AdPilot помогает управлять рекламой по реальным показателям, но не гарантирует CPL, продажи или окупаемость.']
  ],
  finalKicker: 'Без карты и Meta-доступа',
  finalTitle: 'Покажите Алексу свой бизнес',
  finalLead: 'Ответьте на вопросы в Telegram и получите бесплатный стратегический разбор бизнеса: кого привлекать, с каким предложением выходить в рекламу и что проверить первым.',
  finalButton: 'Получить бесплатный стратегический разбор бизнеса',
  legal: 'HABELEV STANISLAV · Stripe €150/месяц до отмены · крипто €150/30 дней без автопродления · рекламный бюджет отдельно.'
};

function renderIsraelRuPage() {
  return renderPage('ru', israelRuCopy)
    .replace('<meta name="robots" content="index,follow,max-image-preview:large">', '<meta name="robots" content="noindex,follow">')
    .replaceAll(`${baseUrl}/ru/`, `${baseUrl}/ru/israel/`)
    .replace('<a class="brand" href="/ru/israel/"', '<a class="brand" href="/ru/"')
    .replace('<link rel="alternate" hreflang="ru" href="https://adpilot.projectstudio-big.net/ru/israel/">', '<link rel="alternate" hreflang="ru" href="https://adpilot.projectstudio-big.net/ru/">')
    .replace('<link rel="alternate" hreflang="x-default" href="https://adpilot.projectstudio-big.net/ru/israel/">', '<link rel="alternate" hreflang="x-default" href="https://adpilot.projectstudio-big.net/ru/">');
}
...
const israelRuDirectory = join(root, 'ru', 'israel');
await mkdir(israelRuDirectory, { recursive: true });
await writeFile(join(israelRuDirectory, 'index.html'), renderIsraelRuPage(), 'utf8');
